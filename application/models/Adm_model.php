...
class Adm_model extends CI_Model
{
	function __construct()
  {
	...
	function orders($filter, $from, $end)
	{
		...
		SELECT concat_ws(' ', u.lastname, u.firstname, u.middlename) as full_name, o.id,o.user_id, o.order_number, o.tmp_user_id, orn.address_id, orn.order_status_id,  sum(o.`count`) as product_count, sum(o.ex_price) as product_price, o.date_time, os.order_status_title  FROM ali_order_numbers as orn
  ...
  function add_user($var)
    ...
    function user_list($vars, $from, $end)
	{
		...
		$sql= "SELECT u.*, s.status_name, g.gender_name, r.name as role_name FROM ".'ali_'."users as u LEFT JOIN ".'ali_'."users_role as r on r.id=u.role_id LEFT JOIN ".'ali_'."genders as g on g.id=u.gender_id LEFT JOIN ".'ali_'."status as s on s.id=u.status_id WHERE u.deleted=0 ".$param;
    ...
    public function get_menu_list($role_id)
    {
		...
		(SELECT am.*, 0 as rel_id, mt.menu_type_name FROM ".'ali_'."admin_menus_id as am_id
		left join ".'ali_'."admin_menus as am on am.menu_id = am_id.menu_id
		LEFT JOIN ".'ali_'."menu_types as mt ON mt.menu_type_id = am_id.menu_type_id
		...
		".'ali_'."admin_menus_id as am_id
		LEFT join ".'ali_'."admin_menus as am on am.menu_id = am_id.menu_id
		LEFT JOIN ".'ali_'."relations as rel ON rel.rel_item_id = am_id.menu_id
...
LEFT JOIN (SELECT am.*  FROM ".$prefix."admin_menus as am  where am.lang_id = ".$this->session->userdata("lang_id")." union SELECT am2.*  FROM ".$prefix."admin_menus as am2 where am2.lang_id = 1  and am2.menu_id not in(SELECT menu_id FROM ".$prefix."admin_menus as am3 where am3.lang_id = ".$this->session->userdata("lang_id").")) as am on am.menu_id = am_id.menu_id WHERE am_id.deleted=0 and am_id.menu_type_id=1 and am_id.parent_id=0")->result_array();
	...
	{
		$result = $this->db->query("Update ".'ali_'."users set active=".$user_status." WHERE id=".$user_id);
		return $result;
	}
