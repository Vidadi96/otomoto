...
class Auth2 extends CI_Controller {
	...
	private function check_function()
	{
		...
		$ip = md5($_SERVER['REMOTE_ADDR']);
		$number = 1;
		$result = $this->auth2_model->check_login($name, $pass);
		if($result)
		{
				$data = array(
		...
		else
		{
			...
			redirect("/auth2/index/wrong_login");
		}
	}
