...
class Adm extends MY_Controller {
	...
	public function index($order_number=0, $address_id=0)
	{
		if($this->input->post())
		{
			$this->universal_model->item_edit_save_where(array("order_status_id"=>$this->input->post("order_status_id")), array("order_number"=>$this->input->post("order_number")), "ali_order_numbers");
		}
		$data=[];
		if(!$order_number)
		{
			...
			$data["address"] = 	$this->universal_model->get_item("ali_addresses", array("address_id"=>$address_id));
			$data["user"] = 	$this->universal_model->get_item("ali_site_users", array("user_id"=>$data["address"]->user_id));
			$data["order_number"] = 	$this->universal_model->get_item("ali_order_numbers", array("order_number"=>$order_number));
			//print_r($data["order"]);
	...
	public function languages()
	{
		if($this->input->post())
		{
			...
			if($error)
			{
				...
				$result = $this->universal_model->add_item($vars, "ali_langs");
				if($result)
				{
		...
		$data["langs"]=$this->universal_model->get_more_item("ali_langs", array("deleted"=>0));
	...
	public function edit_lang($lang_id)
	{
		...
		if($this->input->post())
		{
			...
			if($error)
			{
				...
				$result = $this->universal_model->item_edit_save_where($vars, array("lang_id"=>$lang_id), "ali_langs");
				if($result)
				{
		...
		$data["lang"]=$this->universal_model->get_item("ali_langs", array("deleted"=>0, "lang_id"=>$lang_id));
	...
	function set_active_passive_lang()
	{
		...
		{
			$result = $this->universal_model->item_edit_save("ali_langs", array("lang_id"=>((int)$this->input->post("id"))), array("active"=>((int)$this->input->post("active_passive"))));
			if($result)
			{
	...
	function delete_lang()
	{
		if($this->input->post("id"))
		{
			$this->universal_model->item_edit_save_where(array("deleted"=>1), array("lang_id"=>(int)$this->input->post("id")), "ali_langs");
	...
	public function translations()
	{
		...
		if($this->input->post())
		{
			...
			//print_r($array);
			$result = $this->universal_model->add_more_item($array, "ali_langmeta");
			if($result)
	...
	public function delete_meta_key()
	{
		if($this->input->post("id"))
		{
			$this->universal_model->delete_item(array("meta_key"=>$this->input->post("id")), "ali_langmeta");
			echo '{"msg":"Əməliyyat uğurla baş tutdu. Silindi!", "'.($this->security->get_csrf_token_name()).'":"'.($this->security->get_csrf_hash()).'", "status":"success", "header":"Uğurlu!"}';
		}else {
	...
	public function delete_user()
	{
		...
		$result = $this->adm_model->update_item(array("id"=>$this->input->post("id")), array("deleted"=>1),"ali_users");
		if($result)
		{
	...
	public function menu_permissions()
	{
		...
		if($this->input->post())
		{
			...
			$menu_id = $this->universal_model->add_item($menu_id_vars, "ali_admin_menus_id");
			$menu_vars = array();
			...
			$this->universal_model->add_more_item($relations, "ali_relations");

			$result = $this->universal_model->add_more_item($menu_vars, "ali_admin_menus");
	...
	public function edit_admin_menu($menu_id)
	{
		...
		if($this->input->post())
		{
			$vars = $this->input->post();
			$menu_id_vars = array("icon"=>$vars["icon"], "parent_id"=>$vars["parent_id"], "menu_type_id"=>$vars["menu_type_id"], "order_by"=>$vars["order_by"], "link"=>$vars["link"]);
			$this->universal_model->item_edit_save_where($menu_id_vars, array("menu_id"=>$menu_id), "ali_admin_menus_id");
			...
			$relations=array();
			$this->universal_model->delete_item(array("menu_id"=>$menu_id), "ali_admin_menus");
			$result = $this->universal_model->add_more_item($menu_vars, "ali_admin_menus");

			foreach ($vars["role_id"] as $key => $value) {
				$relations[]=array("item_id"=>$value, "rel_item_id"=>$menu_id, "rel_type_id"=>1);
			}
			$this->universal_model->delete_item_where(array("rel_type_id"=>1, "rel_item_id"=>$menu_id), "ali_relations");
			$this->universal_model->add_more_item($relations, "ali_relations");
		...
		$selected_roles = $this->universal_model->get_more_item("ali_relations", array("rel_item_id"=>$menu_id, "rel_type_id"=>1));
	...
	{
		$this->universal_model->delete_item_where(array("rel_type_id"=>1, "rel_item_id"=>$this->input->post("id")), "ali_relations");
		$this->universal_model->delete_item_where(array("menu_id"=>$this->input->post("id")), "ali_admin_menus");
		$result =  $this->universal_model->delete_item_where(array("menu_id"=>$this->input->post("id")), "ali_admin_menus_id");
		if($result)
		{
			$this->cache->delete_all();
			...
			if($this->form_validation->run() == TRUE)
      {
        ...
        $key = $this->config->item('encryption_key');
  			...
  			$hashed_password = md5(hash('sha512', $salt1.$password.$salt2));
				...
				$result = $this->adm_model->insert_item($user_array, 'ali_users');
		...
		$data['role_list'] = $this->adm_model->select_where('id, name', 'active=1 and deleted=0 and id != 1', 'ali_users_role');
	...
	public function active_passive_admin_user()
	{
		if($this->input->post("id"))
		{
			...
			$result = $this->adm_model->update_item('id='.$id, array('active' => $active), 'ali_users');
	...
	public function delete_admin_user()
	{
		if($this->input->post("id"))
		{
			...
			$result = $this->adm_model->update_item('id='.$id, array('deleted' => 1), 'ali_users');
	...
	public function add_role()
	{
      ...
      $filtered_post = $this->filter_data($post_data);
			...
			$result = $this->adm_model->insert_item($role_array, 'ali_users_role');
		...
		$data['role_list'] = $this->adm_model->select_where('id, name, active', 'deleted=0 and id!=1', 'ali_users_role');
	...
	public function active_passive_role()
	{
		if($this->input->post("id"))
		{
			...
			$result = $this->adm_model->update_item('id='.$id, array('active' => $active), 'ali_users_role');
	...
	public function delete_role()
	{
		if($this->input->post("id"))
		{
			...
			$result = $this->adm_model->update_item('id='.$id, array('deleted' => 1), 'ali_users_role');
