...
<div class="m-content">
    <!--begin::Form-->
    <form action="/rent_admin/edit_car/<?=$this->uri->segment(3); ?>" method="POST" class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed">
                ...
                <div class="m-portlet m-portlet--accent m-portlet--head-solid-bg m-portlet--head-sm" m-portlet="true" id="m_portlet_tools_2">
                    <div class="m-portlet__body">
                            ...
                            <div class="col-md-4">
                              ...
                              <select class="form-control" name="class" id="class">
                                <?php foreach ($classes as $row): ?>
                                  <option <?=($car_data->class == $row->id)?'selected':''; ?> value="<?=$row->id; ?>"><?=$row->class; ?></option>
                                <?php endforeach; ?>
                              </select>
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="form-group m-form__group row" style="">
                            ...
                            <div class="col-md-4">
                              <label for="included_km">Daxil edilmiş km</label>
                              <input class="form-control" type="number" name="included_km" id="included_km" value="<?=$car_data->included_km; ?>">
                            </div>
                            <div class="col-md-4">
                              <label for="surcharge">İcarə dövrünü keçdikdə</label>
                              <input class="form-control" type="number" name="surcharge" id="surcharge" value="<?=$car_data->surcharge; ?>">
                            </div>
                            <div class="col-md-4">
                              <label for="deposit">Depozit</label>
                              <input class="form-control" type="number" name="deposit" id="deposit" value="<?=$car_data->deposit; ?>">
                            </div>
                            <div class="col-md-4">
                              <label for="limitation">Yürüşün məhdudlaşdırılması</label>
                              <input class="form-control" type="number" name="limitation" id="limitation" value="<?=$car_data->limitation; ?>">
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-check">
                                    <input <?=$car_data->delivery?'checked':''; ?> class="form-check-input" name="delivery" type="checkbox" id="delivery">
                                    <label class="form-check-label" for="delivery">Çatdırılma</label>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-check">
                                    <input <?=$car_data->returning?'checked':''; ?> class="form-check-input" name="returning" type="checkbox" id="returning">
                                    <label class="form-check-label" for="returning">Geri götürmə</label>
                                </div>
                            </div>
                            <div class="clearfix"></div>
                        </div>
                        <div class="form-group m-form__group row">
                          ...
                          <div class="col-md-3">
                              <div class="form-check">
                                  <input <?=$car_data->seatventilation?'checked':''; ?> class="form-check-input" name="havalandirilma" type="checkbox" id="havalandırılma">
                          ...
                          <div class="col-md-3">
                              <div class="form-check">
                                  <input <?=$car_data->esp?'baby_chair':''; ?> class="form-check-input" name="baby_chair" type="checkbox" id="baby_chair">
                                  <label class="form-check-label" for="baby_chair">Uşaq oturacağı</label>
                              </div>
                          </div>
                          <div class="col-md-3">
                              <div class="form-check">
                                  <input <?=$car_data->roof_rack?'checked':''; ?> class="form-check-input" name="roof_rack" type="checkbox" id="roof_rack">
                                  <label class="form-check-label" for="roof_rack">Dam üstü baqaj</label>
                              </div>
                          </div>
                          <div class="col-md-3">
                              <div class="form-check">
                                  <input <?=$car_data->insurance?'checked':''; ?> class="form-check-input" name="insurance" type="checkbox" id="insurance">
                ...
                <div class="m-portlet m-portlet--accent m-portlet--head-solid-bg m-portlet--head-sm" m-portlet="true" id="m_portlet_tools_2">
                    ...
                    <div class="m-portlet__body">
                        ...
                        <div class="form-group row">
                            <div class="col-lg-12">
                                <label>Qiymət</label>
                                <input type="number" name="price" class="form-control m-input"  value="<?=$car_data->price; ?>">
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-lg-12">
                        ...
                        <div class="form-group row">
                            <div class="col-lg-12">
                                ...
                                <select class="form-control" name="status">
                                    <option <?=($car_data->status == 0)?'selected':''; ?> value="0">Gözləmədə</option>
                                    <option <?=($car_data->status == 1)?'selected':''; ?> value="1">Aktiv</option>
                                    <option <?=($car_data->status == 2)?'selected':''; ?> value="2">Deaktiv</option>
                                    <option <?=($car_data->status == 3)?'selected':''; ?> value="3">İmtina edilmiş</option>
                                    <option <?=($car_data->status == 4)?'selected':''; ?> value="4">Silinmiş</option>
                                </select>
                            </div>
                        </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    if ($('input[name="added"]').val() == 1)
      toastr["success"]('Uğurla əlavə edildi', "Uğur");
  });
...
$(document).ready(function() {
  ...
  $.get('/rent_admin/add_img/?type=preload&product_id='+product_id, function(result) {
      var preloaded = [];
      ...
      $(document).on('click', '.fileuploader-action-asmain', function (evt) {
          ...
          $(this).closest("li").fadeOut(300, function(){
              ...
              $.post('/rent_admin/add_img/?type=sort&product_id=' + product_id, {
      ...
      $('input.gallery_media').fileuploader({
          ...
          upload: {
              url: '/rent_admin/add_img/?type=upload&product_id='+product_id,
              data: null,
              type: 'POST',
          ...
          onRemove: function(item) {
              // send request
              if (item.data.listProps)
                  $.post('/rent_admin/add_img/?type=remove&product_id='+product_id, {
                      name: item.name,
