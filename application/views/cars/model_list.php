...
<!-- END: Subheader -->
	<div class="m-content">
        ...
        <?php if(isset($status)): ?>
                ...
                <form action="/cars/add_model" method="post">
                  <input type="hidden" id="token" name="<?=$this->security->get_csrf_token_name(); ?>" value="<?=$this->security->get_csrf_hash(); ?>">

                  <div class="form-group m-form__group row">
                    <div class="col-lg-4">
											<label for="mark">Marka</label>
											<input list="mark" class="form-control" required>
											<datalist id="mark">
                        <?php foreach($mark_list as $row): ?>
													<option data-value="<?=$row->id; ?>" value="<?=$row->mark ?>">
                        <?php endforeach; ?>
											</datalist>
                    ...
                    <div class="col-lg-4">
                      <div class="form-group">
                        <label for="model">Model</label>
												<textarea name="model" id="model" class="form-control" required></textarea>
												<span class="m-form__help">Misal üçün: <i>M5,M6,M7</i></span>
                      </div>
                    </div>
        ...
        <div class="m-portlet m-portlet--accent m-portlet--head-solid-bg m-portlet--head-sm" m-portlet="true" id="m_portlet_tools_2">
          ...
          <div class="m-portlet__body">
            <table delete_url="/cars/delete_model/" class="table table-bordered m-table"  active_passive_url="/cars/active_passive_model/">
              <thead>
                <tr>
                  <th>#</th>
                  <th>Model</th>
...
$(document).on('input', 'input[list="mark"]', function(){
  $('input[name="mark_id"]').val('');

  var input = $(this).val();
  $('datalist option').each(function(i) {
    if($(this).val() == input) {
      var mark_id = $(this).attr('data-value');
      $('input[name="mark_id"]').val(mark_id);
    }
  });
});
