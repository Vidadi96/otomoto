...
$(document).ready(function() {
  ...
  if (status == 'edit_error' || status == 'delete_error' || status == 'auction_pin_error')
  ...
  else if (status == 'auction_success')
    toastr.success('Elanınız uğurla hərraca əlavə olundu', 'Uğur');
  else if (status == 'auction_error')
    toastr.error('Xəta baş verdi. Xaiş edirik bir də cəhd edəsiniz', 'Xəta');
  ...
  // } else {
    ...
    $.ajax ({
      ...
      success: function(data){
        ...
        $('.for_auction_calc_price .change').text(((last_price*1)/100).toFixed(0));
...
$(document).on('click', 'input[name="offer_procent"]', function(){
  if ($(this).hasClass('offer_1'))
    ...
    $('.for_auction_calc_price .change').text(((last_price*2)/100).toFixed(0));
  else if ($(this).hasClass('offer_3'))
    $('.for_auction_calc_price .change').text(((last_price*3)/100).toFixed(0));
});
...
/*------ SAVE NAME PHONE IN SESSION ------*/

$(document).on('blur', '.session_save', function(){
   let name = $('input[name="user_name"]').val();
   let user_phone = $('.user_phone').val();

   $.ajax({
     url: '/Pages/save_name_phone_in_session',
     type: 'POST',
     data: {otomoto: token, user_name: name, user_phone: user_phone}
   });
});
