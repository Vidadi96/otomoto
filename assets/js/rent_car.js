...
$(document).on('input', '.class_search', function(){
  var value = $(this).val();

  $('.class_block2').each(function() {
    if (value) {
      if (~$(this).find('.class_block_content span:first-child').text().toLowerCase().indexOf(value.toLowerCase()))
        $(this).show()
      else
        $(this).hide();
    } else {
...
$(document).on('click', '.class_block2', function(){
  ...
  $.ajax ({
      ...
      data: {otomoto: main_token, class: classs},
      success: function(data)
      {
        var res = $.parseJSON(data);
        main_token = res['otomoto'];
...
$(document).on('click', '.mark_list', function(){
  ...
  $.ajax ({
      ...
      success: function(data)
      {
        ...
        for (var i=0; i < res['mark'].length; i++) {
          var countt = res['mark'][i].count?res['mark'][i].count:0;
          html = html + '<div class="popular_mark_block auto_filter" data="'+ res['mark'][i].id +'" data_name="'+ res['mark'][i].mark +'">'
                        + '<div class="popular_mark_block_content">'
                          + '<span>'+ res['mark'][i].mark +'</span>'
                          + '<span>'+ countt +'</span>'
                        + '</div>'
...
$(document).on('click', '.popular_mark_block', function(){
  ...
  var auto_filter = $(this).hasClass('auto_filter')?'auto_filter':'';
  ...
  $.ajax ({
      ...
      success: function(data)
      {
        ...
        var html =  '<div class="popular_model_block ' + auto_filter + '" style="background: #dc3545; width: calc(100% - 10px);" model="" model_name="" mark="'+ mark +'" mark_name="'+ mark_name +'">'
        ...
        for (var i=0; i < res['model'].length; i++) {
          ...
          html = html + '<div class="popular_model_block ' + auto_filter + '" model="'+ res['model'][i].id +'" model_name="'+ res['model'][i].model +'" mark="'+ mark +'" mark_name="'+ mark_name +'">'
                        + '<div class="popular_model_block_content">'
...
$(document).on('click', '.popular_model_block', function(){
  ...
  var auto_click = $(this).hasClass('auto_filter')?true:false;
...
$(document).on('click', '.null_button', function(){
  ...
  $('.filter_button').text('Bütün siniflər');
  $(this).hide();
});
...
$(document).on('change', '.select_mobile', function(){
    ...
    var url = '/pages/rent_car?' + (location.search.slice(location.search.indexOf('?')+1));
    var regEx = /([?&]order)=([^#&]*)/g;

    if (searchParams.has('order'))
        var newurl = url.replace(regEx, '$1=' + $(this).val());
    else
        var newurl = '/pages/rent_car?order='+ $(this).val() +'&' + (location.search.slice(location.search.indexOf('?')+1));
  ...
  if (from < simple_count)
  {
    if ($(window).scrollTop() + window.innerHeight > $(document).height()*2/3)
    {
      if (allow)
      {
        ...
        $.ajax ({
            type: "POST",
            url: "/pages/rent_main_pagination?"  + location.search.slice(location.search.indexOf('?')+1),
...
$(document).on('click', '.class_block', function(){
  let class_id = $(this).attr('class_id');
  $('input[name="class"]').val(class_id);
